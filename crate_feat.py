...
url = 'crate-demo.pth'
#url  = '/jet/home/guntakan/CRATE/checkpoint/CRATE_basemedical-4-ckpt.t7'
device = 'cuda' if torch.cuda.is_available() else 'cpu'
model = CRATEFeat(feat_dim = 768, crate_arch = 'demo',pretrained_path = url, device = device)

def collect_attention_maps(img_dir, image_size, patch_size, layer):
    img_list = sorted(os.listdir(img_dir))[:8]
    attn_list = []
    resized_images = []
...
img_dir = 'lungs_test'
#img_dir = 'braintumor_selected'
#img_dir = '/jet/home/guntakan/braintumor/Testing/glioma'
#img_dir = '/ocean/projects/cis230067p/cteh/cxr14/test/Effusion'
image_size = 480
patch_size = 8
layer = 10
...
fig, axs = plt.subplots(len(imgs),6)
for i, img in enumerate(imgs):
    ...
    for j in range(1,nh):
        ...
        axs[i][j].imshow(attentions[j])
        axs[i][j].axis('off')
    
plt.savefig("image_lungs_pretrained_final.png")
